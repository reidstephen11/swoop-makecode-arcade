...
const cases=[['starter','idle',60],['starter','left',30],['starter','bot',60],['m2','bot',60],['m3','bot',40],['m4','bot',60],['combined','bot',40],['right','bot',20],['m5','bot',35],['zero','bot',12],['fast','bot',15],['white','bot',12],['doublehit','doublehit',4],['blank','bot',12],['countdown','idle',7],['fifth','bot',15]];
...
const results=[];for(let i=0;i<cases.length;i+=4){const batch=await Promise.allSettled(cases.slice(i,i+4).map((x,j)=>run(x,i+j)));for(const r of batch){if(r.status==='fulfilled')results.push(r.value);else console.log('TEST FAILURE',r.reason);}}
